...
st.markdown("<div style='text-align:right; color:gray;'>Developed by <b>Gattu Navaneeth Rao</b></div>", unsafe_allow_html=True)
...
buy_threshold = st.sidebar.number_input("Required % upside for BUY", min_value=0.0, max_value=20.0, value=0.5, step=0.1,
                                        help="Predicted > last close by this % triggers BUY; else HOLD.")
    ...
    if data.empty or 'Close' not in data.columns:
        ...
        if len(X_train) < 10:
            ...
            if lookahead_points <= 0:
                ...
                X_test = [test_data[i - sequence_length:i, 0] for i in range(sequence_length, len(test_data))]
